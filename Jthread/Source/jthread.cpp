...
static long int __jump_interation_context=0XD4E;
struct sigaction __contex__jthread;
struct itimerval __it_jthread;
...
__attr_mutex __attr_mutex_standard;
	...
	if(!__jthread_LIFO_t_objects)
	{
		...
		if(__attr_jthread != NULL)
		{
			...
			__attr_jthread->jump_open_memory->jthread_pointer_function = __thread_function;
                       ...
                       __contex__jthread.sa_flags = SA_RESTART;
                       sigfillset (&__contex__jthread.sa_mask);
                     
                      __it_jthread.it_interval.tv_sec = 0;
                      __it_jthread.it_interval.tv_usec = 1;
                      __it_jthread.it_value = __it_jthread.it_interval;
...
void __jthread_base(__attr_mutex * __attr_jthread)
{
    ...
    if(__attr_mutex_standard.bit_thread_status = __JUMP_JOIN)
...
void __jthread_join(void)
{
    ...
    sigaction (SIGPROF, &__contex__jthread, NULL);
    setitimer (ITIMER_PROF, &__it_jthread, NULL);

    unsigned int __number_thread_random_temp = 0;
    register int __context_get;
    
    for(__context_get=1;__context_get<=__jthread_LIFO_t_objects;__context_get++)
    {
        getcontext (&(__JTHREAD_SYID->OBJECTS[__context_get].jthread_jump));
    }
    abort ();
    
    for(__context_get=1;__context_get<=__jthread_LIFO_t_objects;__context_get++)
    {
         __JTHREAD_SYID->OBJECTS[__context_get].jthread_jump.uc_link = &__JTHREAD_SYID->OBJECTS[__context_get-1].jthread_jump;
        __JTHREAD_SYID->OBJECTS[__context_get].jthread_jump.uc_stack.ss_sp = __JTHREAD_SYID->OBJECTS[__context_get].st;
        __JTHREAD_SYID->OBJECTS[__context_get].jthread_jump.uc_stack.ss_size = sizeof(__JTHREAD_SYID->OBJECTS[__context_get].st);
        makecontext (&__JTHREAD_SYID->OBJECTS[__context_get].jthread_jump,((__jthread_LIFO_t+__context_get-1)->jthread_pointer_function), (__jthread_LIFO_t+__context_get)->thread_data);
    }   
    
    __attr_mutex_standard.bit_thread_status = __JUMP_JOIN;
    __jthread_base(&__attr_mutex_standard);
    
    //swapcontext (&__JTHREAD_SYID->OBJECTS[0].jthread_jump, &__JTHREAD_SYID->OBJECTS[__jthread_LIFO_t].jthread_jump);	
